  ...
  if (typeof tags === "string") {
    try {
      ...
      if (tags.startsWith("[") && tags.endsWith("]")) {
        return tags.slice(1, -1).split(",").map(t => t.trim().replace(/^["']|["']$/g, ""));
      }
      return [tags];
    }
  }
...
function mapPost(result: any): Post {
  return {
    ...result,
...
export class DrizzlePostRepository implements PostRepository {
  ...
  async findMany(query?: ListPostsQuery): Promise<Post[]> {
    const conditions = [];
    ...
    if (query?.search) {
      conditions.push(
        or(
          ...
          like(posts.category, `%${query.search}%`),
          sql`JSON_CONTAINS(${posts.tags}, JSON_ARRAY(${query.search}))`
        )
      );
    }
    ...
    const results = await db.query.posts.findMany({
      where: and(
        ...
        or(
          ...
          like(posts.category, `%${query}%`),
          sql`JSON_CONTAINS(${posts.tags}, JSON_ARRAY(${query}))`
        )
      ),
      orderBy: [desc(posts.createdAt)],
    });
