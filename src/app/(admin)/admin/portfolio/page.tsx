...
import { Plus, Search, Calendar, FolderOpen, Tag, Briefcase } from "lucide-react";
...
export default async function AdminPortfolioDashboard({ searchParams }: PageProps) {
  const params = await searchParams;
  const search = params.search || "";
  const category = params.category || "";
  const status = params.status || undefined;

  // Fetch projects from DI container
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-neutral-900 dark:text-zinc-100 tracking-tight">
            จัดการผลงานผลิต (Portfolio Management)
          </h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            เพิ่ม แก้ไข และบริหารจัดการรูปภาพผลงานสกรีนเสื้อ เสื้อยืดพิมพ์ลาย และสิ่งพิมพ์แบรนด์ต่างๆ
          </p>
        </div>
        <Button
          asChild
      ...
      <div className="bg-white dark:bg-zinc-900/40 border border-neutral-200 dark:border-neutral-800 rounded-xl p-4">
        <form method="GET" className="flex flex-col sm:flex-row items-center gap-3">
          <div className="relative w-full sm:flex-1">
            ...
            <Input
              name="search"
              placeholder="ค้นหาตามหัวข้อผลงาน..."
              defaultValue={search}
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            {(search || category) && (
              ...
              >
                <Link href="/admin/portfolio">ล้างค่า</Link>
              </Button>
            )}
          </div>
        </form>
        ...
        {projects.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <h3 className="mt-4 text-sm font-semibold text-neutral-900 dark:text-zinc-100">
              ไม่พบข้อมูลผลงานผลิต
            </h3>
            <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
              เริ่มต้นเพิ่มประวัติผลงานชิ้นแรกเพื่อนำไปแสดงบนหน้าเว็บไซต์ของคุณ
            </p>
            <div className="mt-6">
              <Button
                asChild
              ...
              >
                <Link href="/admin/portfolio/new">เพิ่มประวัติผลงานชิ้นแรก</Link>
              </Button>
            </div>
          </div>
        ) : (
            ...
            <Table>
              <TableHeader className="bg-neutral-50/50 dark:bg-zinc-950/40">
                <TableRow className="border-neutral-200 dark:border-neutral-800">
                  ...
                  <TableHead className="font-bold">หัวข้อผลงาน</TableHead>
                  <TableHead className="font-bold">หมวดหมู่</TableHead>
                  <TableHead className="font-bold">ชื่อลูกค้า / แบรนด์</TableHead>
                  <TableHead className="font-bold">สถานะ</TableHead>
                  <TableHead className="font-bold">วันที่ผลิต</TableHead>
                ...
                {projects.map((project) => (
                  ...
                  >
                    ...
                    <TableCell className="font-medium align-middle">
                      <div className="flex flex-col gap-0.5 max-w-[250px] sm:max-w-md">
                    ...
                    <TableCell className="align-middle text-sm font-semibold text-zinc-600 dark:text-zinc-300">
                      <div className="flex items-center gap-1.5">
                        <Briefcase className="w-3.5 h-3.5 text-zinc-400" />
                        <span>{project.client || "ทั่วไป"}</span>
                      </div>
                    </TableCell>
                    <TableCell className="align-middle">
                    ...
                    <TableCell className="text-right align-middle">
                      <div className="flex justify-end">
                        <RowActions id={project.id} title={project.title} />
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
